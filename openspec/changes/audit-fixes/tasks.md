...
|------|------|-----------|-------|
| 1 | Shared errors + all three P0 fixes (backlog cap, server callId, CT token) | PR 1 | Base: main; tests per spec scenario; independent scope |
...
- [ ] 1.1 `packages/server/src/errors.ts` — Add `CONFIRMATION_BACKLOG_FULL` + `PAYLOAD_TOO_LARGE` to `GlyphErrorCode`; add `413` + `503` to `ErrorStatus`
  - Dependency for Fix 1 (503) and Fix 5A (413). Design: shared constants, no logic.

## Phase 2: P0 Fixes (PR 1 — security blockers)

- [ ] 2.1 **Confirmation backlog limit** (Fix 1) — `packages/server/src/server.ts`
...
- [ ] 2.2 **Server-generated callId** (Fix 2) — `packages/server/src/server.ts`, `packages/types/src/types.ts`, `packages/core/test/core.test.ts`
...
- [ ] 2.3 **Constant-time token check** (Fix 3) — `packages/server/src/middleware.ts`
...
- [ ] 3.1 **Body size limit** (Fix 5A) — `packages/server/src/server.ts`
...
- [ ] 3.2 **Schema complexity guard** (Fix 5B) — `packages/core/src/json-schema-validator.ts`
...
- [ ] 3.3 **OpenAPI trusted baseUrl** (Fix 4) — `packages/adapters/openapi/src/index.ts`
