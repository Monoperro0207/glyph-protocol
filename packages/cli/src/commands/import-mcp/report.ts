 ...
 */
export async function emitReport(outDir: string, result: ImportResult): Promise<string> {
  const lines: string[] = [
  ...
  const review = result.imported.flatMap((s) =>
    s.cards.filter((c) => c.riskTier !== 'safe').map((c) => ({ server: s.config.name, ...c })),
  )
